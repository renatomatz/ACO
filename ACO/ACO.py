...
class AntColonyOptimizer:
    def __init__(self, ants, evaporation_rate, intensification, alpha=1.0, 
                 beta=0.0, beta_evaporation_rate=0, choose_best=.1):
        """
        Ant colony optimizer. Traverses a graph and finds either the max or 
    ...
    def __str__(self):
        ...
        string += "\nDesigned to optimize either the minimum or maximum" \
                  " distance between nodes in a square matrix that behaves" \
                  " like a distance matrix."
        string += "\n--------------------"
        string += "\nNumber of ants:\t\t\t\t{}" \
                  .format(self.ants)
        string += "\nEvaporation rate:\t\t\t{}" \
                  .format(self.evaporation_rate)
        string += "\nIntensification factor:\t\t{}" \
                  .format(self.pheromone_intensification)
        string += "\nAlpha Heuristic:\t\t\t{}" \
                  .format(self.heuristic_alpha)
        string += "\nBeta Heuristic:\t\t\t\t{}" \
                  .format(self.heuristic_beta)
        string += "\nBeta Evaporation Rate:\t\t{}" \
                  .format(self.beta_evaporation_rate)
        string += "\nChoose Best Percentage:\t\t{}" \
                  .format(self.choose_best)
        string += "\n--------------------"
        string += "\nUSAGE:"
        string += "\nNumber of ants influences how many paths are" \
                  " explored each iteration."
        string += "\nThe alpha and beta heuristics affect how much" \
                  " influence the pheromones or the distance heuristic" \
                  " weigh an ants' decisions."
        string += "\nBeta evaporation reduces the influence of the" \
                  " heuristic over time."
        string += "\nChoose best is a percentage of how often an ant will" \
                  " choose the best route over probabilistically choosing" \
                  " a route based on pheromones."
        string += "\n--------------------"
        if self.fitted:
    ...
    def _initialize(self):
        """
        Initializes the model by creating the various matrices and generating 
        the list of available nodes
        """
        assert self.map.shape[0] == self.map.shape[1], \
                "Map is not a distance matrix!"
    ...
    def _update_probabilities(self):
        """
        After evaporation and intensification, the probability matrix needs 
        to be updated.  This function does that.
        """
    ...
    def _choose_next_node(self, from_node):
        """
        Chooses the next node based on probabilities.  If p < p_choose_best, 
        then the best path is chosen, otherwise it is selected from a 
        probability distribution weighted by the pheromone.
    ...
    def _evaluate(self, paths, mode):
        """
        Evaluates the solutions of the ants by adding up the distances 
        between nodes.

        :param paths: solutions from the ants
        :param mode: max or min
        :return: x and y coordinates of the best path as a tuple, the 
            best path, and the best score
        """
    ...
    def _evaporation(self):
        ...
        Also evaporates beta if desired.
        """
    ...
    def fit(self, map_matrix, iterations=100, mode='min', 
        ...
        """
        Fits the ACO to a specific map. This was designed with the 
        Traveling Salesman problem in mind.
        ...
        if verbose: print("Beginning ACO Optimization with {} iterations..." \
            .format(iterations))
        ...
        for i in range(iterations):
            ...
            for ant in range(self.ants):
                current_node = self.set_of_available_nodes[
                    np.random.randint(0, len(self.set_of_available_nodes))
                ]
                start_node = current_node
                while True:
                    # can't this be a for loop?
                    path.append(current_node)
                    self._remove_node(current_node)
                    if len(self.set_of_available_nodes) != 0:
                        current_node = self.set_of_available_nodes[
                            self._choose_next_node(current_node)
                        ]
                    else:
                        break
            ...
            if i == 0:
                ...
                if (mode == 'min') and (best_score < best_score_so_far):
                    best_score_so_far = best_score
                    self.best_path = best_path
                elif (mode == 'max') and (best_score > best_score_so_far):
                    best_score_so_far = best_score
                    self.best_path = best_path
            ...
            if verbose: print(f"Best score at iteration {i}:" 
                              f"{round(best_score, 2)};" 
                              f"overall: {round(best_score_so_far, 2)}" 
                              f"({round(time.time() - start_iter)}s)")

            if (best_score == best_score_so_far) \
               and (num_equal == early_stopping_count):

                self.stopped_early = True
                print(f"Stopping early due to {early_stopping_count}"
                      " iterations of the same score.")
                break
        ...
        if verbose: print(
            "ACO fitted.  Runtime: {self.fit_time // 60} minutes."
    ...
    def plot(self):
        ...
        if not self.fitted:
            ...
            ax.text(.8, .6,
                    f"Ants: {self.ants}\n"
                    f"Evap Rate: {self.evaporation_rate}\n"
                    f"Intensify: {self.pheromone_intensification}\n"
                    f"Alpha: {self.heuristic_alpha}\n"
                    f"Beta: {self.heuristic_beta}\n"
                    f"Beta Evap: {self.beta_evaporation_rate}\n"
                    f"Choose Best: {self.choose_best}\n\n"
                    "Fit Time: {}m{}".format(self.fit_time // 60,
